...
本插件是 [qwenpaw-longtext-input](https://github.com/levodoubt/qwenpaw-longtext-input) 在 DeepSeek Harness 上的重写，**不是移植**：原版是 QwenPaw 的前端插件，靠 DOM 注入按钮 + `DataTransfer` 伪造 `<input type="file">` 把正文塞进附件流；DSH 有正规的槽位系统，且本插件按需求改走"落盘 + `@` 引用"，不复用原版任何代码。只有行为规格（按钮位置、标题→文件名、取消时确认）是继承的。
